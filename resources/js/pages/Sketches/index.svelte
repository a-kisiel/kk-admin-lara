...
<div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
    ...
    >
        <div class="list-top-bar">
            ...
            <a href="/sketches/add" class="list-add">
                Add Sketch <Plus />
            </a>
        </div>
        <div class="list piece-list">
            {#each props.sketches as sketch}
            <PieceCard
                piece={sketch}
                type="sketch"
            />
            {:else}
            <div class="list-else">No sketches yet.</div>
            {/each}
        </div>
        {#if props.sketches?.length > 0}
        <div class="total-row">Total: {props.total_sketches}</div>
        {/if}
        <div class="list-pagination">
            <Pagination.Root count={props.total_sketches} perPage={20}>
